...
function draw()
    ...
    action_queue = {} --table of actions that should be performed as a batch per frame
...
function increaseSV(time, multiplier)
    ...
    local sv = map.GetScrollVelocityAt(time) or utils.CreateScrollVelocity(0, 1)
...
function getScrollVelocityIndex(sv)
    for i, velocity in pairs(map.ScrollVelocities) do
        if velocity == sv then return i end
    end
end
